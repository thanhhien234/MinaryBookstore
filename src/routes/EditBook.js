...
function EditBook() {
  ...
  useEffect(() => {
    ...
    fetch(editUrl, {
      ...
      .then(res => {
        console.log('res', res);
        setBookInfo(res);
        setData({ ...data, id: res.id, salePrice: res.salePrice, category: res.category, conditions: res.conditions, detail: res.detail, longitude: res.longitude, latitude: res.latitude, address: res.address, startDate: res.startDate, endDate: res.endDate, imageIdList: res.imageList });
        setImages(res.imageList.map((item, index) => ({ id: item.id, position: index, fileUrl: item.url, fileObject: null })));
      })
  ...
  useEffect(() => {
    ...
    setData(prevData => ({
      ...prevData,
      imageIdList: images.map(image => image.id)
    }));
  }, [images]);
    ...
    <div className="edit-main-container">
      ...
      {bookInfo.bookGetRes && (
        <div className="edit-book-content">
          <div className="edit-book-info">
            ...
            <div>
              <span>판매가</span>
              <input type="number" id="salePrice" defaultValue={bookInfo.salePrice} onBlur={(e) => setData({ ...data, salePrice: parseInt(e.target.value) })} />원
            </div>
            <div>
              <span>카테고리</span>
              <select className="edit-category" value={bookInfo.category} onChange={(e) => setData({ ...data, category: e.target.value })}>
      ...
      {bookInfo.conditions && (
        <div className="book-condition-wrapper">
          <h3>책 상태가 어떤가요?</h3>
          <ConditionRadioList radioEditable={true} initialConditions={bookInfo.conditions}
        ...
        <div className="book-rent-period-wrapper">
          ...
          <div className="rent-period-input">
            <div>
              ...
              <input type="date" id="fromTime" defaultValue={bookInfo.startDate} onChange={(event) => setData({ ...data, startDate: event.target.value })} />
            </div>
            <div>
              <label htmlFor="toTime">종료일:</label>
              <input type="date" id="toTime" defaultValue={bookInfo.endDate} onChange={(event) => setData({ ...data, endDate: event.target.value })} />
            </div>
          </div>
        </div>
      )}
      <div className="book-real-image-wrapper">
        ...
        <div className="book-real-image-inner">
          {images.map((image, index) => (
            <UploadedImage key={index} position={index} uploadedImage={image.fileUrl} setImages={setImages} images={images} />
          ))}
          {images.length < 5 && (
      ...
      <div className="book-description-wrapper">
        <h3>추가 설명을 입력하세요</h3>
        <textarea id="descriptionInput" rows="6" cols="60" defaultValue={bookInfo.detail} onBlur={(e) => setData({ ...data, detail: e.target.value })} />
      </div>
